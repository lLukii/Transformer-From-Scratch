...
class MultiHeadAttention(nn.Module):
  def __init__(self, num_heads: int, d_model: int, attention_size: int, mask=False):
    # man how tf do u vectorize ts 😭
    """
    num_heads: number of attention heads
    mask: whether to use masked attention
    d_model: dimensionality of input
    attention_size: dimensionality of Q, K, and V vectors
    """
  ...
  def forward(self, x: torch.tensor):
    ...
    '''
    batch_size, seq_len, d_model = x.size()
    attention = torch.zeros(batch_size, seq_len, self.num_heads, self.attention_size)
